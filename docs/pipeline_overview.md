...
The Documents Generation is oriented by generated document. So, there are multiple pipelines, each focused in a specific document. The documents and their logicar order are defined below. A more in depth explanation of each agent for each pipeline can be found at [Agents and Components](./agents_and_components.md)
