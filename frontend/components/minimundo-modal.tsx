...
import { X, BookOpen, Loader2, CheckCircle, RefreshCcw } from "lucide-react"
import { Textarea } from "@/components/ui/textarea"
import { useState, useEffect } from "react"
import { Label } from "@/components/ui/label" // Import Label

interface MinimundoModalProps {
  isOpen: boolean
  data: string
  isProcessing: boolean // This refers to the initial generation, not regeneration within modal
...
export default function MinimundoModal({
  isOpen,
  data,
  isProcessing,
  onConfirm,
  ...
  useEffect(() => {
    ...
    setRegeneratePrompt("") // Clear prompt when data changes (new artifact or new regeneration)
  ...
  const handleRegenerateClick = async () => {
    ...
    // Call the parent's onRegenerate function with the prompt
    await onRegenerate(regeneratePrompt)
    // The parent will update the 'data' prop, which will trigger the useEffect to update editedContent and reset isRegenerating
  }
          ...
          {isProcessing ? (
            ...
            <>
              <Textarea
                ...
                className="min-h-[400px] font-mono text-sm border border-border p-4 rounded-lg resize-none focus:ring-primary focus:border-primary bg-background text-foreground"
